  ...
  return (
    <div className="container homepage">
      ...
      <section id='section-2' className='mt-15 mb-15'>
        <div className='left'>
          <h3 className='text-center mb-10' id='title'>Nền tảng kinh doanh ngành làm đẹp cho spa, thẩm mỹ viện, salon, clinic/đồng bộ và quản lý vận hành tại một nơi</h3>
          {
            Section2Contents.map((content, index) =>
              <div className='mb-7 max-w-600 c-pointer' onClick={() => setActiceContent(index === activeContent ? -1 : index)}>
                <div className='flex align-center jc-between mb-7' >
                  <h4 className={activeContent === index ? 'active' : ''}>{content.title}</h4>
                  <Icon name={activeContent === index ? 'arrow-up' : 'arrow-down'}></Icon>
                </div>
                <div className='divider' />
              </div>
            )
          }
          <h6 className='max-w-600 normal'>{Section2Contents[activeContent]?.content}</h6>
          <div className='subcontent max-w-800 mt-5'>
            {Section2Contents[activeContent]?.subcontent.map(content =>
